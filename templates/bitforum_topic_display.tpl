	...
	<div class="floaticon">
		{if $print_page ne 'y'}
			{if $gBitUser->hasPermission( 'p_bitforum_edit' )}
				<a title="{tr}Remove this bitforum{/tr}" href="{$smarty.const.BITFORUM_PKG_URL}edit.php?t={$gContent->mInfo.bitforum_topic_id}">{biticon ipackage="icons" iname="accessories-text-editor" iexplain="Edit BitForum"}</a>
			{/if}
			{if $gBitUser->hasPermission( 'p_bitforum_remove' )}
				<a title="{tr}Remove this bitforum{/tr}" href="{$smarty.const.BITFORUM_PKG_URL}remove_bitforum.php?forum_id={$gContent->mInfo.bitforum_id}">{biticon ipackage="icons" iname="edit-delete" iexplain="Remove BitForum"}</a>
			{/if}
		{/if}<!-- end print_page -->
	</div><!-- end .floaticon -->
