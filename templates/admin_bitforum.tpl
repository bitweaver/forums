	...
	{jstabs}
		{jstab title="Home BitForum"}
			{legend legend="Home BitForum"}
				...
				<div class="row">
					{formlabel label="Home BitForum (main bitforum)" for="homeBitForum"}
					{forminput}
						<select name="homeBitForum" id="homeBitForum">
							{section name=ix loop=$bitforums}
								<option value="{$bitforums[ix].bitforum_id|escape}" {if $bitforums[ix].bitforum_id eq $home_bitforum}selected="selected"{/if}>{$bitforums[ix].title|escape|truncate:20:"...":true}</option>
							{sectionelse}
